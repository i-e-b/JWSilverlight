...
namespace ComposerCore {
	...
	public struct TimeCode : IComparable, IComparable<TimeCode>, IEquatable<TimeCode> {
		...
		public TimeCode (int hours, int minutes, int seconds, int frames, SmpteFrameRate rate) {
			...
			frameRate = rate;
			absoluteTime = Smpte12mToAbsoluteTime(timeCode, frameRate);
		}
		...
		public TimeCode (string timeCodeAndRate) {
			...
			frameRate = SmpteFrameRate.Smpte2997NonDrop;

			if (rate == "29.97" && time.IndexOf(';') > -1) {
				frameRate = SmpteFrameRate.Smpte2997Drop;
				...
				frameRate = SmpteFrameRate.Smpte2997NonDrop;
					...
					case "25":
						...
						break;
					case "23.98":
						frameRate = SmpteFrameRate.Smpte2398;
						break;
					case "24":
			...
			absoluteTime = Smpte12mToAbsoluteTime(time, frameRate);
		}
		...
		public int HoursSegment {
			get {
				string timeCode = AbsoluteTimeToSmpte12M(absoluteTime, frameRate);
		...
		/// </summary>
		/// <returns>
		/// The minute component of the current TimeCode structure. The return value ranges from 0 through 59.
		/// </returns>
		...
		/// </summary>
		/// <returns>
		///  The second component of the current TimeCode structure. The return value ranges from 0 through 59.
		/// </returns>
		...
		/// </summary>
		/// <returns>
		/// The frame component of the current TimeCode structure. The return value depends on the framerate selected for this instance. All frame counts start at zero.
		/// </returns>
		...
		public double TotalHours {
			get {
				long framecount = AbsoluteTimeToFrames(absoluteTime, frameRate);
		...
		/// </returns>
		public double TotalSeconds { get { return absoluteTime; } }
		...
		/// </returns>
		public long TotalFrames { get { return AbsoluteTimeToFrames(absoluteTime, frameRate); } }
		...
		public static TimeCode operator - (TimeCode time1, TimeCode time2) {
			double time = time1.absoluteTime - time2.absoluteTime;

			if (time < MinValue) {
				throw new OverflowException("Timecode out of range");
			}
		...
		public static bool operator != (TimeCode time1, TimeCode time2) {
			if (Math.Floor(Math.Pow(10, 2) * time1.absoluteTime) != Math.Floor(Math.Pow(10, 2) * time2.absoluteTime)) {
				return true;
			}
		...
		/// </exception>
		public static TimeCode operator + (TimeCode time1, TimeCode time2) {
			var time3 = new TimeCode(time1.absoluteTime + time2.absoluteTime, time1.FrameRate);
			if (time3.absoluteTime >= MaxValue) {
		...
		public static bool operator < (TimeCode time1, TimeCode time2) {
			return time1.absoluteTime < time2.absoluteTime;
		}
		...
		public static bool operator <= (TimeCode time1, TimeCode time2) {
			if ((time1.absoluteTime < time2.absoluteTime) || (time1 == time2)) {
				return true;
			}
		...
		public static bool operator == (TimeCode time1, TimeCode time2) {
			return Math.Floor(Math.Pow(10, 2) * time1.absoluteTime) == Math.Floor(Math.Pow(10, 2) * time2.absoluteTime);
		}
		...
		public static bool operator >= (TimeCode time1, TimeCode time2) {
			return (time1.absoluteTime > time2.absoluteTime) || (time1 == time2);
		}
		...
		public static TimeCode FromHours (double hours, SmpteFrameRate rate) {
			double absoluteTime = hours * TicksPerHourAbsoluteTime;
		...
		public static TimeCode FromMinutes (double minutes, SmpteFrameRate rate) {
			double absoluteTime = minutes * TicksPerMinuteAbsoluteTime;
			return new TimeCode(absoluteTime, rate);
		}
		...
		public static TimeCode FromFrames (long frames, SmpteFrameRate rate) {
			double abs = FramesToAbsoluteTime(frames, rate);
		...
		public static TimeCode FromTicks27Mhz (long ticks27Mhz, SmpteFrameRate rate) {
		...
		public override string ToString () {
			return AbsoluteTimeToSmpte12M(absoluteTime, frameRate);
		}
		...
		public string ToString (SmpteFrameRate rate) {
			return AbsoluteTimeToSmpte12M(absoluteTime, rate);
		}
		...
		public long ToTicks27Mhz () {
			return AbsoluteTimeToTicks27Mhz(absoluteTime);
		}
		...
		public long ToTicksPcrTb () {
			return AbsoluteTimeToTicksPcrTb(absoluteTime);
		}
		...
		/// <param name="absoluteTime">The absolute time to convert from.</param>
		/// <returns>A string that contains the correct format.</returns>
		private static string AbsoluteTimeToHMS (double absoluteTime) {
			long framecount = AbsoluteTimeToFrames(absoluteTime, SmpteFrameRate.Unknown);
			int hours = Convert.ToInt32((framecount / 108000) % 24);
			int minutes = Convert.ToInt32((framecount - (108000 * hours)) / 1800);
			int seconds = Convert.ToInt32(((framecount - (1800 * minutes) - (108000 * hours)) / 30));
